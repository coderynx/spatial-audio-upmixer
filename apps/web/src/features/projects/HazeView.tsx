  ...
  React.useEffect(() => {
    ...
    const draw = (time: number, { width, height }: { width: number; height: number }) => {
      ...
      for (const voice of voices) {
        ...
        const targetHeightLevel =
          voice.heightAngle !== null ? (spectrum?.level ?? 0) : 0;
        ...
        const next: SmoothedVoice = previous
          ? {
              ...
              radius:
                previous.radius +
                (targetRadius - previous.radius) * Math.min(1, delta * 6),
              heightRadius:
                voice.heightAngle !== null
                  ? lerpAngle(
              ...
              heightLevel:
                previous.heightLevel +
                (targetHeightLevel - previous.heightLevel) *
                  Math.min(1, delta * 8),
            }
          : {
              angle: voice.angle,
              radius: targetRadius,
              heightRadius: voice.heightAngle ?? 0,
              level,
        ...
        const color =
          propsRef.current.colors[voice.stem] || canvasTheme.stemFallback;
        ...
        const blobRadius =
          (radius * 0.32 + next.level * radius * 0.28) *
          voice.sizeScale *
          (currentSelected === voice.stem ? 1.1 : 1);
        ...
        if (voice.heightAngle !== null && emphasis > 0.005) {
          const heightPoint = polar(
            center,
            heightRingRadius,
            next.heightRadius,
          );
          const heightBlob = 6 + next.heightLevel * 16;
          const heightGradient = ctx.createRadialGradient(
            heightPoint.x,
            heightPoint.y,
            0,
            heightPoint.x,
          ...
          heightGradient.addColorStop(
            0,
            `rgba(${r}, ${g}, ${b}, ${(0.5 + next.heightLevel * 0.5) * emphasis})`,
          );
