...
def test_narrowing_the_layout_retargets_a_delivery_it_cannot_carry(
    ...
    with TestClient(create_app(settings)) as client:
        ...
        assert manifest["format"]["type"] == "multichannel"


def test_widening_the_layout_retargets_a_codec_it_cannot_carry(tmp_path, monkeypatch):
    settings = Settings(
        data_dir=tmp_path,
        database_url=f"sqlite:///{tmp_path / 'retarget_codec.db'}",
        worker_count=1,
    )
    monkeypatch.setattr("upmixer_web.worker.WorkerManager.start", lambda _self: None)
    monkeypatch.setattr("upmixer_web.worker.WorkerManager.stop", lambda _self: None)
    with TestClient(create_app(settings)) as client:
        ...
        assert created.status_code == 201
        assert created.json()["manifest"]["format"]["codec"] == "flac"

        # Only the layout changes; 7.1.4 delivers 12 channels, past FLAC's cap.
        response = client.put(f"/api/v1/projects/{created.json()['id']}/settings", json={
            "manifest": {
                "version": "1.0.0",
                "engine": {"mode": "stem", "stems": ["Vocals"]},
