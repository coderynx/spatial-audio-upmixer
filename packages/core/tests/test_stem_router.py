...
def _drum_over_bed(n: int = 96_000, sr: int = 48_000) -> np.ndarray:
    ...
    t = np.arange(n) / sr
    hit_len = int(0.06 * sr)
    shape = np.minimum(np.arange(hit_len) / (0.001 * sr), 1.0)
...
def test_transient_duck_defaults_off_and_leaves_routing_bit_identical():
    audio = _drum_over_bed()
    fmt = FORMAT_MAP["7.1.4"]

    assert UpmixConfig().stem_transient_duck == 0.0
    off = StemRouter(UpmixConfig(), fmt, 48_000).route({"Other": audio}, len(audio))
    explicit = StemRouter(
        UpmixConfig(stem_transient_duck=0.0), fmt, 48_000
    ).route({"Other": audio}, len(audio))

    for channel, signal in off.items():
        assert np.array_equal(signal, explicit[channel]), channel


def test_transient_duck_attenuates_onsets_in_the_diffuse_sends_only():
    sr = 48_000
    audio = _drum_over_bed()
    fmt = FORMAT_MAP["7.1.4"]
    hit_len = int(0.06 * sr)
    starts = [k * sr // 2 for k in range(1, 2 * len(audio) // sr)]
    hits = np.concatenate([np.arange(s, s + hit_len) for s in starts])
    sustain = np.concatenate([np.arange(s + 9_000, s + 21_000) for s in starts[:-1]])

    off = StemRouter(UpmixConfig(), fmt, sr).route({"Other": audio}, len(audio))
    on = StemRouter(
        UpmixConfig(stem_transient_duck=0.7), fmt, sr
    ).route({"Other": audio}, len(audio))

    def db(channel: str, index: np.ndarray) -> float:
        before = float(np.sum(off[channel][index] ** 2))
        after = float(np.sum(on[channel][index] ** 2))
        return 10.0 * math.log10(after / before)

    # Onsets lose ground in the surround and height sends, sustain does not.
    for channel in ("SL", "TFL"):
        assert db(channel, hits) < -3.0, channel
        assert db(channel, hits) < db(channel, sustain) - 3.0, channel

    # The front bed keeps the whole transient: with the diffuse sends quieter,
    # phase 9's loudness renormalization hands it back, not away.
    assert db("FL", hits) > 0.0
