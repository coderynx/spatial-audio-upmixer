...
export function SpatialScene({
  ...
  return <div className="overflow-hidden rounded-lg border bg-slate-950 p-3 text-slate-100">
    <div className="mb-2 flex items-center justify-between text-xs text-slate-300"><span>Speaker routing</span><button className="hover:text-white" onClick={() => onSelectStem(null)}>{selectedStem || "Aggregate output"}</button></div>
    <svg className="h-[350px] w-full" viewBox="0 0 100 100" role="img" aria-label="Speaker routing map">
    ...
    {channels.includes("LFE") && <div className="mt-1 rounded bg-slate-800 px-2 py-1 text-xs">LFE {selectedStem ? `${(route.LFE || 0).toFixed(2)} send` : "bus"}</div>}
  </div>;
}
