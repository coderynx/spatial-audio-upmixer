...
export function ProjectsPage({ projects, loading, error, onRefresh, onDelete }: { projects: Project[]; loading: boolean; error: string | null; onRefresh: () => void; onDelete: (project: Project) => void }) {
  return <main className="w-full p-3 sm:px-6 sm:py-5">
    ...
    {loading ? <div className="h-48 animate-pulse rounded-md border bg-muted/30" /> : projects.length === 0 ? <div className="grid min-h-64 place-items-center rounded-lg border border-dashed p-6 text-center"><div><FolderPlus className="mx-auto mb-3 h-9 w-9 text-muted-foreground" /><h2 className="font-semibold">Create your first project</h2><p className="mt-1 max-w-sm text-sm text-muted-foreground">Import tracks once, separate stems in the background, then keep shaping the mix.</p><Button className="mt-5" asChild><Link to="/projects/new">New project</Link></Button></div></div> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
