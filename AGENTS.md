...
Key modules include `config.py` (`UpmixConfig`), `formats.py` (channel definitions), `manifest.py` (YAML/JSON jobs), `analysis/`, `decomposition/`, `routing/`, `io/`, `mastering/`, `upmix/`, and `eval/` (objective separation evaluation: SDR/fullness/bleedless against a reference corpus). Keep the public compatibility shims `mastering_comp.py`, `mastering_bass.py`, and `mastering_eq.py` intact.
...
- `python3 -m pytest -m perf -k eval -s` runs the separation evaluation harness and prints the per-stem SDR/fullness/bleedless report.
