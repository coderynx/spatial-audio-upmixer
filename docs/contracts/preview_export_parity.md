...
| D36 | Phase 13 replaced that detector with three of them behind a Linkwitz-Riley crossover, so the audio thread now runs eight extra biquads and two extra envelope pairs per routed stem at full depth. The D33/D35 lesson says bench before believing it is free. | Measured, and it is nearly free: alternating `bench:engine` runs of the phase 11 and phase 13 artifacts differ by **2-3% in mean** and under 0.05x in p99 on every case, with no case changing verdict. The machine used was not idle (load average ~3), so the absolute numbers in that session sit above budget on `measuring (exact, paused)`, `measuring (fast excerpt, playing)`, `binaural` and `transaural` — identically at both artifacts, which is what makes the delta the readable figure and the absolutes not. A quiet-machine re-run is owed before the stage is defaulted on. At the shipped default of 0.0 the crossover never runs and the stage is skipped as before. |
