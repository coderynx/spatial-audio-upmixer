...
| D37 | The Haze and Elevation displays read `stem_spectrum`, which measures the *raw decoded stem*, so nothing the routing stage does to a stem was ever visible in them — the transient duck could pull a send down 6 dB with the blobs unmoved. The obvious fix, reading the ducker's live gain, is wrong for a different reason: routing runs a full look-ahead horizon ahead of the emit position (P1: up to 300 ms), so the display would have flashed the reduction long before the hit that caused it was audible. | Fixed: `MultibandDucker` exposes the deepest band gain per sample, `StemRouteState` accumulates it into a per-block trace, and the engine queues that trace per stem and reads it back over the same trailing `METER_WINDOW_FRAMES` window at the emit position that every other meter uses. The views scale a stem's level by `1 - duckedFraction(route) * (1 - duck)` — FL/FR/C carry the dry bed and never move — and Haze's height ring, which is nothing but ducked send, by the gain in full. `stream_equivalence.rs::the_duck_readout_lands_with_the_transient_not_ahead_of_it` pins the alignment. |
| D38 | The transient duck's gain is `1.0 - depth * score` with `score` clamped to 1.0, so at **depth exactly 1.0** a saturating onset lands on gain 0.0 — the band is annihilated, not ducked. With three bands that means one band vanishing while its neighbours ride on at partial gain, heard as the send changing colour rather than level ("hi-hats and crashes lose some frequencies", the D36 report). Measured as a discontinuity, not a slope: on the reporter's own stems the §9.1 tilt swing runs 0.68 / 2.55 / 1.72 dB (Crash / Ride / Hi-Hat) at depth 0.90 and 1.79 / 3.71 / 3.18 at 0.99, then jumps to **11.41 / 4.90 / 10.74 at 1.00**. The broadband ducker has the same endpoint (0.63 → 3.02 on the Crash), so this is the duck's own singularity, not the band split's. | Fixed at the root rather than by capping the control: `DUCK_MIN_GAIN = 0.1` floors the per-band gain at −20 dB in `TransientDucker::tick`, which every caller — both bindings, all three bands, preview and export — routes through. Depth 1.0 now scores **0.70 / 2.70 / 1.80**, monotone and saturating, and costs 0.04–0.4 dB of maximum ducking depth. A diffuse send 20 dB down is gone by ear; a send at exactly zero is a different filter. The slider keeps its full 0–1 range, no manifest or CLI bound moves, and every depth ≤ 0.9 is bit-identical to before (`.max()` is a no-op there). `full_depth_floors_the_gain_instead_of_nulling_the_band` pins both the floor and that it is actually reached. **This also reframes D36**: its 23.58 dB crash figure was measured at depth 1.0, i.e. exactly at the singularity. Off the endpoint the split costs ~1.8-2.8x broadband rather than 3.8x. That does not by itself overturn the revert, but the number it was reverted on was the endpoint's, not the split's. |
| D39 | The duck's detector window sat *inside* ordinary crest variation instead of above it, and its two halves were tuned as one number. `DUCK_THRESHOLD_RATIO`/`DUCK_FULL_RATIO` were 1.25/2.5 — 1.9 to 8.0 dB over the running mean — while measurement over real stems puts the fast/slow ratio of sustained material at p75 ~1.2 and p90 ~1.5, and percussive onsets at 16-45. The threshold therefore caught the top quartile of normal signal peakiness: a ride wash scored a mean 0.120 against a snare's 0.126, i.e. the stage ducked the wash **as hard as the hit it exists to separate from the wash**, and the resulting continuous mid-scale gain motion (Ride ducking 27.6% of the time but saturating only 5.0% — the rest a 60 ms glide, several per second) is heard as compression. Reported as "sounds compressed, only usable at 0.2-0.3". | Fixed in two steps, and the second is the lesson. Moving the window to 2.5/8.0 fixed the duty cycle — Guitar 9.3%→0.9%, Lead Vocals 20.7%→3.6%, Ride 27.6%→5.0%, Snare holding 9.5% — and made the stage **inaudible**, because widening the span from 1.25 to 5.5 also cut how hard a qualifying onset ducks: snare saturation fell to 2.5% against the original's 9.5%. The two are independent — `active%` is set by the threshold alone and is identical at 2.5/3.5, 2.5/4.0, 2.5/6.0 and 2.5/8.0 — so the fix is a high threshold with a *narrow* span. Shipped at **2.5/4.0**: selectivity kept (Snare/Ride mean 1.05 → 2.21), saturation restored to 5.8%, and real separation at last — at depth 0.7 a Snare loses 7.53 dB against a Ride's 1.90, a 4x ratio where the original window was 1.05x. Cymbal tilt swing ≤1.58 dB at every depth. Depth 0.0 untouched and still the default. |
| D40 | Every fixture in the duck's test suite was too weak to reach the detector at all, so D39's defect was invisible to 188 passing tests and the corrected threshold *broke* them. The stimuli were sub-millisecond spikes (which never move the 1.5 ms attack envelope), sitting 13-17 dB over their bed (where the score is a ratio and needs ~30), spaced 250 ms apart (where the reference envelope's own 250 ms holds the ratio down to ~6.6 whatever the level). Two tests were additionally passing on the cold-start sample alone, where the reference envelope is still zero, the ratio is unbounded and *any* signal saturates. | Fixed by rebuilding the fixtures around what the detector actually measures: 30 ms decaying strikes, ~30 dB over the bed, spaced 500 ms, scanned from the second hit so the cold start cannot carry the assertion. `hit_train_over_bed` documents all three constraints; `_drum_over_bed` and the two stream-side fixtures carry the same shape. This is `feedback_multiband_validation_corpus` again, one layer down: phase 13 learned that narrowband stimuli cannot exercise a per-band stage, and the same suite could not exercise the *detector* either. A duck test that never crosses the threshold measures nothing. |
