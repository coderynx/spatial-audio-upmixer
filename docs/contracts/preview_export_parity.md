...
| P7 | **Apple Spatial monitor render.** The Tauri-only Apple Spatial mode hands every multichannel mastered speaker bed to a `PHASEPushStreamNode` backed by `PHASEAmbientMixerDefinition`; stereo uses direct `AVAudioEngine` output. The PCM before PHASE is the same native-bed output the export path owns, except PHASE ignores LFE: the output adapter restores LFE's +10 dB replay gain and folds it equally into FL/FR, then clears the ignored channel. System spatialization, head tracking, and this output adaptation are deliberately outside export parity, like any external ADM renderer. This mode never changes persisted mix or mastering parameters. Meter presentation waits for PHASE's render callback plus the output node's public presentation latency; PHASE exposes no separate spatializer latency. | Pre-renderer PCM remains algorithm-identical; spatialized output is monitor-only and not an export reference. Visual timing may lead by PHASE's undisclosed processing latency. |
