...
| LFE lowpass cutoff (also `render_binaural`/`render_crosstalk`'s `lfe_cutoff_hz` — see **Ledger D22**) | 120 Hz | `lfe_cutoff_hz` | `LFE_LOWPASS_HZ` | 1 |
...
| D22 | A review of per-stem LFE routing (`separation/stem_router.py`) found: (1) most stems had no `"LFE"` key in `DEFAULT_ROUTING`/`ZONE_ROUTING` at all, so the send amount was implicit and had no UI control; (2) `ZONE_ROUTING["height_front"/"height_back"]["Bass"/"Kick"]` were explicit empty `{}` routes — `_routing_for` does not fall back to `DEFAULT_ROUTING` for an explicit override, so those stems produced silence, not reduced height content, in any multichannel-source height zone; (3) `binaural/renderer.py::_lfe_lowpass` hardcoded `cutoff_hz=120.0`/`butter(2, ...)` instead of reading `UpmixConfig.lfe_cutoff_hz`/`lfe_filter_order`, so a configured cutoff change silently had no effect on binaural or transaural renders, and its 2nd-order filter disagreed with `StemRouter`'s 4th-order default and the web preview's two cascaded biquads; (4) `apps/api/.../routing.py::routing_for_scene` excluded `ChannelLabel.LFE` from its candidate speaker list entirely, so a scene-positioned stem got no LFE send at all rather than the stem's configured/default amount. | Fixed. Every `DEFAULT_ROUTING`/`ZONE_ROUTING["front"/"surround"]["Other"]` entry now has an explicit `"LFE"` weight (0.0 where sub-120 Hz content isn't meaningful). `Bass`/`Kick` height-zone routes now send `{"FL": 0.45, "FR": 0.45, "LFE": ...}` instead of `{}`. `render_binaural`/`render_crosstalk` gained `lfe_cutoff_hz`/`lfe_filter_order` parameters, threaded from `cfg` at both delivery call sites — binaural/transaural now track the configured cutoff exactly like `StemRouter` and the web preview. `routing_for_scene` now writes the stem's `stem_routing[...]["LFE"]` value (falling back to the new `default_lfe_send()` accessor) onto the positional mapping after ranking, without including LFE in the constant-power speaker selection itself. The per-stem LFE amount remains the existing `stem_routing[stem]["LFE"]` field (no new config field) — exposed via a web `StemControls` slider (`ProjectDetailPage.tsx`) and CLI `--stem-lfe STEM=AMOUNT[,...]`. New coverage: `test_stem_router.py` (every default stem has an explicit LFE key, LFE bus scales with weight, height-zone bass/kick reach FL/FR+LFE), `test_binaural.py::test_lfe_lowpass_follows_configured_cutoff`, `apps/api/tests/test_project_routing.py` (positioned stem gets default/explicit LFE), `apps/cli/tests/test_cli_flags.py` (new file — merge-not-clobber, manifest precedence, negative-value rejection), `ProjectDetailPage.test.tsx` (slider shown/hidden by layout, writes `stem_routing`). Also fixed in `components/ui/slider.tsx`: `aria-label`/`aria-labelledby` were accepted as props but only applied to Radix's `Root`, never forwarded to the `Thumb` that actually carries `role="slider"` — silently unlabeled for every existing caller (`Front to back`/`Floor to height` included) until this change's LFE slider test exposed it. |
