...
| P7 | **Apple Spatial monitor render.** The Tauri-only Apple Spatial mode hands every multichannel mastered speaker bed to `AVSampleBufferAudioRenderer`; stereo uses direct `AVAudioEngine` output. The PCM before Apple's renderer is the same native-bed output the export path owns; system spatialization, head tracking, and output adaptation are deliberately outside export parity, like any external ADM renderer. This mode never changes persisted mix or mastering parameters. | Pre-renderer PCM remains algorithm-identical; spatialized output is monitor-only and not an export reference |
