...
| D18 | Not a discrepancy: de-duplicating a web-side copy. The 32-tap Hann-windowed-sinc 4× true-peak kernel (Tier-3, deliberately *not* `loudness.py::_TRUE_PEAK_FIR_4X` — that gap stays, bounded by §5's 1.0 dBTP tolerance) existed as two hand-synced web copies: `masteringProfiles.ts::buildTruePeakKernel` (app-side) and `apps/web/public/truePeakKernel.js` (worklet-side), kept bit-for-bit by `limiterWorklet.test.ts` because an AudioWorklet module can't import the app bundle. The worklet also hardcoded `SAFETY_MARGIN_DB = 0.1`, duplicating core's `limiter.py::_SAFETY_MARGIN_DB`. | Made the app-side function the single web source. `audioEngine.ts` computes the kernel once via `buildTruePeakKernel()` and passes it into the `"limiter-processor"` node as `processorOptions.truePeakKernel` (a plain number array — structured-clone-safe); the worklet reads it from its constructor options and builds no kernel of its own. `truePeakKernel.js` was deleted (nothing else imported it — the old comment's `loudness.worklet.js` never existed). `_SAFETY_MARGIN_DB` is now served by `engine_constants()` as `safety_margin_db`, threaded through `ServedEngineConstants`/`EngineConstants`/`resolveEngineConstants` and passed as `processorOptions.safetyMarginDb`. `limiterWorklet.test.ts` lost its now-meaningless cross-copy equality; it now asserts the kernel's numeric properties (32-tap, symmetric, Hann-zeroed endpoints) against the single `buildTruePeakKernel` and that the worklet builds its detector from `processorOptions.truePeakKernel`. `useStemPreview.test.tsx` additionally asserts the limiter node is constructed with a non-empty served kernel + margin. The structural `OVERSAMPLE = 4` stays worklet-local (fixed, not a tuning value). No output drift: `measureBufferTruePeakDbtp` still uses the same single `buildTruePeakKernel`, so the golden diff (§5) is byte-identical. See §3. |
