...
| P7 | **Apple Spatial monitor render.** The Tauri-only Apple Spatial mode hands every multichannel mastered speaker bed to a `PHASEPushStreamNode` backed by `PHASEAmbientMixerDefinition`; stereo uses direct `AVAudioEngine` output. The rendered native-bed PCM before monitor trim is the same output the export path owns, except PHASE ignores LFE: the output adapter restores LFE's +10 dB replay gain, folds it equally into FL/FR, and clears the ignored channel. The native adapter then applies the transport monitor volume plus a transparent −3 dB PHASE-only calibration downstream of core meters/calibration and before PHASE; neither belongs to the export programme. PHASE derives speaker directions from the standard `AVAudioChannelLayout`; system spatialization and output adaptation remain monitor-only. The preview's loudness pass measures that pre-PHASE PCM because no post-PHASE capture path exists; its readout and correction therefore do not claim the final spatialized acoustic loudness. | Pre-renderer PCM remains algorithm-identical except for the documented LFE monitor fold and the native PHASE-only monitor trim; spatialized output and its unmeasured final loudness are not an export reference |
