...
`upmixer/eval/` and the precondition the roadmap sets on those changes
...
## Metrics (`upmixer/eval/metrics.py`)
...
## Deterministic settings (`upmixer/eval/harness.py`)
...
model weights (`tests/test_eval_metrics.py`); real evaluation runs bind
`separate_for_eval` via `functools.partial` (`tests/test_eval_harness.py`).

## Running it

```bash
# Metric math + harness plumbing — deterministic, no model download.
python3 -m pytest tests/test_eval_metrics.py -q
...
python3 -m pytest -m perf -k eval -s
```
...
- **Model swaps (roadmap 1.2, 2.3, 2.4)** — comparing a candidate model
  ...
  replaces anything in `upmixer/separation/stem_plan.py`.
