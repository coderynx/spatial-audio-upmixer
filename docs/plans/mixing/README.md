...
| 13 | `phase13_multiband_duck.md` | Split the phase 11 duck into 3 Linkwitz-Riley bands so cymbal wash keeps flowing to the heights through snare hits. Structural replacement, `depth` stays the only knob, budget-gated before wiring. **Done, default off** — through a snare hit a 6–12 kHz wash loses 1.99 dB where the broadband duck cost it 8.78 dB, and the split costs 2–3% of the audio-thread mean. The bench session was not on a quiet machine, so its absolutes are unusable and only the phase-11-vs-13 delta is reported; ledger D36. Listening A/B still outstanding, see `phase13_report.md` §7. |
