...
| 11 | `phase11_content_aware_routing.md` | Roadmap 4.1: archaeology gate on the dead stem-analyzer modules, then revive (transient/sustain send split, default-off) or delete. **Done** — the gate splits: both modules deleted (their whole-file mechanism is what the removal condemned, and the preview cannot run it); rung 1 (a causal transient/sustain duck on the diffuse send inputs) shipped, default 0.0, and was later removed in full. See `phase11_report.md` §1 for the archaeology. |

Phases 12–14 extend content-aware routing beyond phase 11's send split. The
