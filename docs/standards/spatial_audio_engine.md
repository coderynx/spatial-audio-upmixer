...
1. A parametric spherical-head HRTF model per virtual-loudspeaker direction
   (piecewise Woodworth ITD formula for front and rear hemispheres +
   frequency-dependent head-shadow ILD: a lowpass plus a 700 Hz high shelf
   on the contralateral path, so the interaural
