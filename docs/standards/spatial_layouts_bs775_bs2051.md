...
| bed panning | MDAP (`stem_panner.py`). The image renders as virtual sources every `VIRTUAL_SOURCE_STEP_DEG` across `azimuth ± width/2` (one when `width` is 0). Every source is panned by VBAP onto a facet of the speakers' convex hull; the gain vectors sum and the map is L2-normalized. `diversity` blends that image toward equal energy in every positional speaker before normalization; `center_level_db` then trims C independently. Sends below `MINIMUM_SEND` are dropped and the rest renormalized. |
